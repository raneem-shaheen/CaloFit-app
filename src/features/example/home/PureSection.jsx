...
export default function PureSection() {
  const { pureFeatures, dishes, loading } = useHomeData()
  ...
  if (loading) {
    return (
      <>
        ...
        <TopSellingSection dishes={dishes} loading={loading} />
      </>
    )
  }
      ...
      <section className="bg-[#FAF8F5] py-16 px-6 lg:px-12">
        <div className="max-w-7xl mx-auto">
          ...
          {pureFeatures.length > 0 ? (
            ...
            <div className="rounded-3xl border border-dashed border-gray-200 bg-white/50 py-10 text-center text-sm text-gray-500">
              Pure ingredients data is unavailable right now.
            </div>
          )}
        </div>
      </section>
